    ...
    getAll(params = null) {
        ...
        if (params) {
            ...
            for(let sortkey of sortKeys)
            {
                ...
                {
                    let compteurPermu = 0;
                    for(let i= 0; i < filteredAndSortedObjects.length-1;++i){
                        let firstObject = filteredAndSortedObjects[i];
                        let nextObject = filteredAndSortedObjects[i+1];
    
                        if(sortkey.asc){
                            if(this.innerCompare(firstObject[sortkey.key],nextObject[sortkey.key]) >0)
                            {
                                filteredAndSortedObjects[i] = nextObject;
                                filteredAndSortedObjects[i+1] = firstObject;
                                ++compteurPermu;
                            }
                        }
                        else{
